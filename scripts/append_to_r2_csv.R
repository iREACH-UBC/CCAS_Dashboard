...
guess_delim <- function(path) {
  # Read a short sample and choose the most frequent among common delimiters
  line <- tryCatch(readr::read_lines(path, n_max = 1), error = function(e) "") 
  if (length(line) == 0) return(",")
  cand <- c("," = ",", "\t" = "\t", ";" = ";", "|" = "|")
  counts <- vapply(cand, function(d) stringr::str_count(line, fixed(d)), integer(1))
  names(which.max(counts))
}

read_any <- function(path, encoding, col_names = TRUE) {
...
headers_match <- function(df, expected_names) {
...
load_input_for_sensor <- function(sensor_id) {
  if (mode == "template") {
    ...
    df_all <- tryCatch(
      read_any(opt$input, encoding = opt$encoding, col_names = TRUE),
...
merge_and_upload <- function(df_in, sensor_id) {
  ...
  split(df_in, df_in$.__yyyymm) %>% imap(function(chunk, ym) {
    ...
    # Download existing (if any), trying headered read first; fallback to headerless and assign names
    ...
    if (object_exists(opt$bucket, key)) {
      ...
      df_existing <- tryCatch(
        read_any(existing_path, encoding = opt$encoding, col_names = TRUE),
        error = function(e) NULL
      )
      
      # If names don't look like our incoming header, assume headerless file and re-read without names
      if (is.null(df_existing) || !headers_match(df_existing, names(chunk))) {
        message("[info] existing object likely has no header; re-reading and assigning incoming column names")
        df_existing <- read_any(existing_path, encoding = opt$encoding, col_names = FALSE)
        
        if (ncol(df_existing) == ncol(chunk)) {
          names(df_existing) <- names(chunk)
        } else {
          message("[warn] existing column count (", ncol(df_existing),
                  ") != incoming (", ncol(chunk), "). Assigning generic names; union will still keep all columns but alignment may be positional.")
          names(df_existing) <- paste0("V", seq_len(ncol(df_existing)))
        }
      }
    } else {
    ...
    # Align columns (keep all; prefer incoming order)
    all_cols <- unique(c(names(chunk), names(df_existing)))
